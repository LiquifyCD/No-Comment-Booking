...
Events use monotonic IDs in a 100-entry bounded buffer. Admins use `/api/live`; regular users use `/api/status`, which omits detailed events. Both maintain one same-origin SSE connection and perform only a bounded recovery snapshot if streaming fails. Date minimums are recalculated in the configured IANA timezone for every backend validation and slot filter. The server forces a 15-second polling interval without exposing it in the UI, disables reservation-only mode, and accepts the explicit automatic-booking choice.

`MonitorJob` owns the canonical status object returned by bootstrap, snapshots, SSE, and mutating API responses. The header, status card, metric, footer, buttons, and open dialogs render that same object. Only the temporary reconnecting state is client-generated; the next snapshot restores the authoritative server state. Booking progresses through match, reservation, payment, and confirmed summary states. If payment returns a booking ID but the final summary cannot be confirmed, the status becomes action-required and payment is not retried.
