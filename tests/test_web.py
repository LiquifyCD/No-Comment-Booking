...
class LocalWebTests(unittest.TestCase):
    ...
    def test_health_and_security_headers(self):
        ...
        self.assertEqual({"status": "ok", "mode": "local", "version": "2.9.3"}, response.json())
    ...
    def test_frontend_has_wizard_with_booking_choice_and_without_visible_interval(self):
        ...
        self.assertNotIn('id="statusBadge"', page)
        self.assertNotIn('id="connectionBadge"', page)
        self.assertNotIn('id="modeBadge"', page)
        self.assertIn('id="footerDescription"', page)
        self.assertIn("function renderStatus(status, authoritative = true)", script)
        self.assertIn('["#statusTitle", "#metricEvent", "#footerStatus"]', script)
        ...
        self.assertIn("const savedLicenceId = Number(SCANNING_STATES.has(state.status?.code)", script)
